...
class Desafio(models.Model):
    ...
    def get_status_display(self):
        # Adicione a lógica para determinar o status com base no número de perguntas respondidas
        perguntas_respondidas = self.flashcards.filter(respondido=True).count()

        if perguntas_respondidas >= self.quantidade_perguntas:
            return 'Concluído'
        else:
            return 'Em Progresso'
